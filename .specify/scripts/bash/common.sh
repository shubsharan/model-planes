...
read_init_option() {
    ...
    if command -v jq >/dev/null 2>&1; then
        value=$(jq -r --arg key "$key" '.[$key] // empty' "$file" 2>/dev/null) || value=""
    elif command -v python3 >/dev/null 2>&1; then
        value=$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1])).get(sys.argv[2], ""))' "$file" "$key" 2>/dev/null) || value=""
    fi
    [[ -n "$value" ]] && printf '%s' "$value" || printf '%s' "$default"
}

resolve_content_dir() {
    local repo_root="$1" key="$2" default="$3" path
    path=$(read_init_option "$repo_root" "$key" "$default")
    [[ "$path" = /* ]] || path="$repo_root/$path"
    printf '%s' "$path"
}

get_features_dir() { resolve_content_dir "${1:-$(get_repo_root)}" feature_dir docs/features; }
...
resolve_feature_name_for_slug() {
    ...
    if [[ -d "$features_dir" ]]; then
        for dir in "$features_dir"/[0-9][0-9][0-9][0-9]-"$slug"; do
            [[ -d "$dir" ]] && matches+=("$(basename "$dir")")
        done
        if ((${#matches[@]} > 1)); then
            echo "Error: multiple feature artifacts match slug $slug" >&2
            return 1
        elif ((${#matches[@]} == 1)); then
            printf '%s\n' "${matches[0]}"
            return 0
...
get_feature_paths() {
    ...
    # A Git feature branch is the active-work source of truth. Non-Git callers
    # may provide an explicit directory for compatibility with standalone use.
    local feature_dir
    if [[ "$has_git_repo" == "true" ]]; then
        ...
        echo "ERROR: SPECIFY_FEATURE_DIRECTORY is required outside a Git repository" >&2
        return 1
    fi
